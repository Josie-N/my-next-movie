...
// usernameParam; const json = await axios.get(url);  return json.data; }
...
//
//   const url = baseURL + '?' + currentPageParam + '&' + moviePageLimitParam + '&' + listNameParam + '&' +
// usernameParam; const json = await axios.get(url);  return json.data; }  export const postToMovieList = async (id,
// username, listName) => { const movieIdParam = `movieId=${id}`;  const listTypeParam = `list=${listName}`; const
// usernameParam = `username=${username}`;  const url = baseURL_2 + '?' + movieIdParam + '&' + listTypeParam + '&' +
// usernameParam; const response = await axios.post(url);  return response.data; }
